...
describe("V1 shop/listing/read-sensitive tools", () => {
  ...
  it("returns an empty paginated listing response without fabricating records", async () => {
    const client = createMockClient();
    vi.mocked(client.getListingsByShop).mockResolvedValue({ count: 0, results: [] });

    const result = await handlers.etsy_list_shop_listings(
      { shop_id: "123", state: "active", limit: 10, offset: 20 },
      client
    );
  ...
  it("maps auth, rate-limit, and Etsy 5xx failures from V1 tools safely", async () => {
    const cases = [
      {
        error: new EtsyAuthError("oauth-refresh-secret", "invalid_token"),
        expected: "Etsy API authentication error",
        hidden: "oauth-refresh-secret",
      },
      {
        error: new EtsyRateLimitError("bearer-secret-rate-limit", 60),
        expected: "Etsy API rate limit error",
        hidden: "bearer-secret-rate-limit",
      },
      {
        error: new EtsyApiError("Etsy API error: 503 Service Unavailable", 503, { token: "api-secret" }),
        expected: "Etsy API error (503)",
        hidden: "api-secret",
      },
    ];
    ...
    for (const failure of cases) {
      const client = createMockClient();
      vi.mocked(client.getListingFull).mockRejectedValue(failure.error);
      const server = new EtsyServer({ apiClient: client });
      ...
      expect(result.isError).toBe(true);
      expect(text).toContain(failure.expected);
      expect(text).not.toContain(failure.hidden);
    }
  });
